  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isEmailExist) {
        return next(
          new ErrorHandler(
            "User with same email or mobile number already exists.",
            409,
          ),
        );
      }
...
export const login = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.findOne({ email: emailEdited }).select(
        "password",
      );
      ...
      const loggedInUser: IUser = await User.findById(user._id).select(
        "-password",
      );
